...
class Test():
    def __init__(self, solution_class):
        self.sol = solution_class

    def run_tests(self, cases):
        for case in cases:
            t1 = timeit.default_timer()
            res = self.sol.maxSubArray(nums=case)
            t2 = timeit.default_timer()
            test_passed = True if res else None
            print(f'Test passed: {test_passed}. Time elapsed: {t2-t1}')
        return test_passed
...
class Solution:
    def maxSubArray(self,  nums: List[int]) -> int:
        if not len(nums):
            return None
        subarray_length = 1
        subarray_baseindex = 0
        subarray = [nums[0]]
        highest_sum = None
        while subarray_length <= len(nums):
            if highest_sum is not None:
                if sum(subarray) > highest_sum:
                    highest_sum = sum(subarray)
            else:
                highest_sum = sum(subarray)
            if subarray_baseindex+subarray_length-1 == len(nums)-1:
                ...
                subarray_baseindex += 1
            subarray = [nums[i] for i in range(
                subarray_baseindex,  subarray_baseindex+subarray_length)]
        return highest_sum
